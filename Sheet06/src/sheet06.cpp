...
/*void calculateHistogram(cv::Mat &img, cv::Mat &hist, cv::Rect &bb) {
    ...
    cv::calcHist(&t, 1, channels, cv::Mat(), hist, 2, histSize, ranges);
    //cv::normalize(hist, hist, 0, 1, cv::NORM_MINMAX, -1, cv::Mat());
}*/
...
public:
    ParticleFilter(){numptl=200; mu_x=0; mu_y=0; sigma=5; particles.resize(numptl); cumulFit.resize(numptl);};
    ~ParticleFilter(){};
...
void ParticleFilter::evaluateCumulFeat() {
    cv::Rect best_bb;
    float best_likelihood = -1;
    std::vector<float> weights(numptl);
    sumFit = 0;
    ...
    for ( auto &particle : particles ) {
        ...
        sumFit += particle.fitness;
        weights[i] = particle.fitness; i++;

        if ( (particle.likelihood(sigma) < best_likelihood) or (best_likelihood == -1) ) {
